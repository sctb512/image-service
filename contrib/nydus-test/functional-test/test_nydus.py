...
def test_pseudo_fs(nydus_anchor, nydus_image: RafsImage, rafs_conf: RafsConf):
    ...
    # FIXME: `lookup` still has no inode offset.
    os.listdir(nydus_anchor.mountpoint)
...
def test_certain_files_prefetch(
    ...
    rafs_conf.set_rafs_backend(Backend.BACKEND_PROXY, image=nydus_scratch_image)
    rafs_conf.dump_rafs_conf()
    ...
    nc = NydusAPIClient(rafs.get_apisock())
    before = nc.get_backend_metrics()
    file = open(os.path.join(nydus_anchor.mountpoint, target_file), "rb")
    file.read(50)
    file.close()
    after = nc.get_backend_metrics()

    logging.info("total backend read %d", before["read_amount_total"])
    assert before["read_amount_total"] == after["read_amount_total"]
